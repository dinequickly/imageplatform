...
# Try to read token from environment variable first (for cloud deployment)
# Fall back to token.txt for local development
...
if not token:
    try:
        ...
        raise ValueError("Canvas token not found! Set CANVAS_TOKEN environment variable or create token.txt")

headers = {
    'Authorization': 'Bearer ' + token
}

CANVAS = 'https://nulondon.instructure.com/api/v1/'
